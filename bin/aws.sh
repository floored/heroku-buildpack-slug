...
# An S3 bucket. Change the values of $AWS_SLUG_BUCKET and REQUEST_REGION below 
...
POST_POLICY='{"expiration":"'$EXPIRE'","conditions": [{"bucket":"'$$AWS_SLUG_BUCKET'" },{"acl":"'$ACL'" },["starts-with", "$key", "'$STARTS_WITH'"],["eq", "$Content-Type", "application/octet-stream"],{"x-amz-credential":"'$AWS_ACCESS_KEY'/'$REQUEST_DATE'/'$REQUEST_REGION'/'$REQUEST_SERVICE'/aws4_request"},{"x-amz-algorithm":"'$ALGORITHM'"},{"x-amz-date":"'$REQUEST_TIME'"}]}'
...
curl --silent \
  ...
  -F "file=@"$FILE_TO_UPLOAD http://$$AWS_SLUG_BUCKET.s3.amazonaws.com/$SLUG_DIR
